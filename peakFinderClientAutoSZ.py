...
import h5py
from utils import *
import libpressio # available since ana-4.0.50-py3
...
def calcPeaks(args, detarr, evt, d, ps, detectorDistance, nevent, ebeamDet, evr0, evr1, fargs):
    ...
    if facility == 'LCLS':
        ...
        if len(d.peakFinder.peaks) >= args.minPeaks and \
           ...
           d.peakFinder.maxRes >= args.minRes:
            ...
            #-- compress and decompress here 
            #00
            comp = make_compressor( d.peakFinder.peaks[:,0:3], fargs)
            compressed = comp.encode(d.peakFinder.calib)
            decompressed = np.zeros_like(d.peakFinder.calib)
            decompressed = comp.decode(compressed, decompressed)
            # Write image in cheetah format
            img = ps.getCheetahImg(decompressed)
...
def runclient(args):
    ...
    for nevent in np.arange(len(times)):
        ...
        if type(fargs) == type(None):
            # set up before the loop
            d.peakFinder.generateMask(evt)
            pressio_mask = d.peakFinder.combinedMask # check mask name
            fargs = setup_compressor_args(fopts, pressio_mask)
            
        calcPeaks(args, detarr, evt, d, ps, detectorDistance, nevent, ebeamDet, evr0, evr1, fargs)
