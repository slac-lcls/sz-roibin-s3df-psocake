...
def convert_peaks_to_cheetah(s, r, c):
    ...
    #print('converting ------------------------------', type(s))
    #print('s',s,'r',r,'c',c)
    
    if isinstance(s, np.float64) or isinstance(s, np.ndarray):
        s = s.astype('int')
        r = r.astype('int')
        c = c.astype('int')

    numAsicsPerQuad, nrows, ncols = (8, 185, 388)

    row2d = (s % numAsicsPerQuad) * nrows + r  # where s%8 is a segment in quad number [0,7]
    col2d = (s // numAsicsPerQuad) * (ncols) + c  # where s/8 is a quad number [0,3]
